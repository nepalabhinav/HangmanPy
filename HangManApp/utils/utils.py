def random_country():
    ...
    return choice(
        ['Afghanistan', 'Aland Islands', 'Albania', 'Algeria', 'American Samoa', 'Andorra', 'Angola', 'Anguilla',
         ...
         'Sri Lanka', 'Sudan', 'Suriname', 'South Sudan', 'Svalbard and Jan Mayen', 'Swaziland', 'Sweden',
...
def indexes_to_value(list1, indexes, value):
    list2 = list1
    for i in indexes:
        if i < len(list1):
            list2[i] = value
...
def replace(list1, from_val, to_val):
    index_list = all_indexes(list1, from_val)
    indexes_to_value(list1, index_list, to_val)
